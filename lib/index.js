...
function keyhasher(query){
  ...
  , key = objecthash(keyData);
  debug('Hashed key [ %s ] with data %j', key, keyData);
  return key;
}
...
module.exports = function(Query, Promise, options){
  ...
  Query.prototype.cache = function(ttl){
    ...
    if(/^(find)(?![a-z]+(update))/ig.test(this.op)&&ttl) {
      debug('Caching query %j', this._mongooseOptions);
      debug('\tQuery explained as %j', this.explain());
      ...
      caches.remove(hashCacheKey(this));
    }
    return this;
  };
